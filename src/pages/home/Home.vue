    ...
    <div class="h-full w-full bg-body flex flex-row justify-center items-center p-4 overflow-hidden">
        ...
        <div class="h-full w-2/12 mr-4">
            ...
            <div class="h-56 rounded-lg w-full bg-module flex flex-col justify-center items-center relative px-4">
                ...
                <div class="h-20 w-20 rounded-full shadow border-2 border-aux" :style="'background-image: url(/img/profile.jpg); background-repeat: no-repeat; background-size: cover; background-position: center;'"></div>
                <span class="text-xl text-white font-semibold text-center leading-none mt-6">Vicente Carrascosa</span>
            ...
            <div class="h-auto rounded-lg bg-module mt-4 overflow-hidden">
                <div v-for="(el,index) in optionsMenu" :key="index" class="w-full border-b border-body p-6 flex flex-row justify-start items-center cursor-pointer" :class="{'bg-box': optionSelected == el}" @click="selectOption(el)">
                    <span class="text-white text-lg font-semibold uppercase">{{el.title}}</span>
